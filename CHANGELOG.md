## 0.3.0

- Updated Android Maven distribution from native source `0.3.0`.
- Added identify email prefill in the report form and reduced accidental shake opens.
- Deprecated native attachment staging helpers and aligned the recommended public API to the core 5 methods.

## 0.2.0

- Updated Android Maven distribution from native source `0.2.0`.
- Added issue frequency selector support in bug report flow.
- Added EN/TR localization with automatic English fallback.
- Includes shake sensitivity tuning and report delivery reliability improvements.

## 0.1.5

- Updated Android binary distribution from native source `0.1.5`.
- Aligned public Maven package version to `0.1.5`.

## 0.1.1

- Restored automatic screenshot capture before opening feedback flow.
- No public API changes.

## 0.1.0

- Simplified initialization API (`token` + optional `shakeToOpenEnabled`).
- Removed public `baseUrl`, `reportPath`, and `autoAttachScreenshot`.
